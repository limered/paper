...
| 12 | **Single shared colour palette across paper, edges, and UI.** Source of truth: `valleyfold/palette/paper-pixels-1x.png` (22 swatches). `valleyfold/src/Render/Palette.cs` names the swatches the renderer uses. Edge colours follow Yoshizawa convention (warm = mountain, cool = valley); creases sitting under upper paper layers render in a desaturated cool grey so the visible top-layer creases stay legible. | Cosy illustration look (Pillar 1) needs one harmonious palette, not per-system ad-hoc colours. Greyed under-creases keep the "real paper stack" cue without competing with the active fold. |
